<section class="panel" id="panel_2">
    <?php
        $result = mysqli_query($con, "select event_id from user_events where username!='$username' order by timestamp desc;");
        while($event_id = mysqli_fetch_array($result)) {
            $result_t = mysqli_query($con, "select user_2 from friends where user_1='$username';");
            while($user_2 = mysqli_fetch_array($result_t))  {
                ...
                echo "
                    <article class=\"post\">
                        <div class=\"post_pic\" style=\"display: inline; float: left; margin-top: 1vh;\"><img src=\"/assets/profile-pictures/$user_2[0].jpg\" style=\"height: 4vh; display: inline; border-radius: 100px;\" /></div>
